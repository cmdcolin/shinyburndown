...
server <- function(input, output) {
  fetchGithubTable <- reactive(
    {
      if (input$orgrepo == "") {
        data.frame(
          start = character(), end = character(), title = character(), start_date = character(),
          end_date = character(), months = character(), issue_number = character(),
          stringsAsFactors = F
        )
      } else {
        res <- c()
  ...
  vals <- reactiveValues(p1 = NULL, p2 = NULL, p3 = NULL, p4 = NULL, p5 = NULL, p6 = NULL)
  ...
  output$plot1 <- renderPlotly(
    {
      ...
      vals$p1 <- ggplot(my_table) +
        geom_linerange(
          aes(
          ymin = end_date, ymax = start_date, x = months, color = issue_number,
          size = I(1),
          text = title
        )
        ) +
  ...
  output$plot2 <- renderPlotly(
    {
      ...
      vals$p2 <- ggplot(my_table) +
        geom_linerange(
          aes(
          ymin = end_date, ymax = start_date, x = issue_number, color = months,
          text = title, size = I(1)
        )
        ) +
        scale_color_gradientn(colours = rainbow(5)) +
  ...
  output$plot5 <- renderPlotly(
    {
      ...
      vals$p5 <- qplot(months, data = my_table) +
        scale_y_log10() + theme_bw() + theme(panel.grid = element_blank()) +
  ...
  output$plot6 <- renderPlotly(
    {
      ...
      total <- rowSums(
        outer(days, my_table$start, ">") &
          outer(days, my_table$end, "<")
      )
  ...
  output$table <- renderDataTable(
    {
      ...
      format(
        head(
          my_table[order(-my_table$months),
